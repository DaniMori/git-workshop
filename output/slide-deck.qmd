...
-   Notice {renv} messages in the console. Explain that what it means:
    ...
    versions) reproducible. This file contains a list of the "packages and their
    versions" to make a project reproducible. But it does not install those
    packages.

-   After the repository is cloned, we have to tell it to "install" all the
    packages of the environment, with their versions.
:::
...
::: nonincremental
-   Download <https://bit.ly/gitws-nb>
...
-   **NOTE**: To avoid opening the file link in the same tab as the
    presentation, press `CTRL + click`. After clicking the link, direct
    attention to button "Download raw file" in GitHub.
...
::: nonincremental
-   Download <https://bit.ly/gitws-data>
...
A new line with a `!` in front of the file path can be added.
...
(The only available file with changes, at this point, will be "renv.lock"; not
very "meaningful" changes.)

\[VIEW FILE:\]

We can "manually revert" changes in a commit by restoring a previous version of
the file, or "copying" what we want to recover and pasting it in the current
